...
// Sensors
gps LGPS = gps(PORT14, -12.5, -7, distanceUnits::cm, 270);
gps RGPS = gps(PORT19, 12.5, -7, distanceUnits::cm, 90);
// DualGPS GPS = DualGPS(LGPS, RGPS, Inertial, vex::distanceUnits::cm);
optical OpticalTop = optical(PORT4);
optical OpticalBottomRight = optical(PORT6);
optical OpticalBottomLeft = optical(PORT2);
distance DistanceMiddle = distance(PORT3);
// optical MiddleSensor = optical(PORT7);
// IMU: one inertial — chassis.Gyro (below) and smartdrive must use the same port as the physical sensor.
inertial Inertial = inertial(PORT8);
// Left Drive
motor LeftDriveA = motor(PORT5, ratio6_1, true); //f
motor LeftDriveB = motor(PORT16, ratio6_1, true); //f
motor LeftDriveC = motor(PORT6, ratio6_1, false); //f
motor_group LeftDrive = motor_group(LeftDriveA, LeftDriveB, LeftDriveC);
// Right Drive
motor RightDriveA = motor(PORT15, ratio6_1, false); //f
motor RightDriveB = motor(PORT17, ratio6_1, false); //f
motor RightDriveC = motor(PORT7, ratio6_1, true); //f
motor_group RightDrive = motor_group(RightDriveA, RightDriveB, RightDriveC);

// Intake and Belt motors
motor Intake = motor(PORT20, ratio6_1, false); //Intake in
// motor StageTwo = motor(PORT8, ratio6_1, false); //SecondStage
// motor StageThree = motor(PORT11, ratio6_1, false); //ThirdStage
// motor Score = motor(PORT12, ratio6_1, false); //Score
...
// trackWidth = 12.0 inches (distance between left and right wheels)
// wheelBase = 0.0 for tank drive (distance between front and back wheels, not applicable for tank)
smartdrive Drivetrain = smartdrive(LeftDrive, RightDrive, chassis.Gyro, 13.5, 13.5, 0.0, distanceUnits::in, 0.75);
...
static void runPhase1(int phaseMillis) {
  ...
  // Waypoint loop: start -> (150,150) -> (-150,-150) -> (150,150) -> (-150,-150) -> ...
...
void driverControl(void) {
  ...
  while (true) {
    ...
    if (Controller.ButtonB.pressing()) {
      ...
      printCurrentLocationOnController();
      wait(500, msec);
    }
    if (Controller.ButtonX.pressing()) {
      waitUntil(!Controller.ButtonX.pressing());
      selectThreeBalls(BallBlue);
      wait(500, msec);
    }
    if (Controller.ButtonR1.pressing()) {
      waitUntil(!Controller.ButtonR1.pressing());
      // collectThreeBallsWithPath(BallBlue);
      collectTwoBallsWithPath(BallBlue);
      wait(500, msec);
    }
  }
}
...
int main() {
  ...
  // StageTwo.setVelocity(30, percent);
  // StageThree.setVelocity(30, percent);
